...
describe('ControlPanel', () => {
  ...
  it('should open profile menu when clicking profile button', () => {
    ...
    const profileBtn2 = component.find(IconButton).at(3);
    profileBtn2.simulate('click', { currentTarget:<button/> });
    //assert the props open of Mobile Profile Menu is true
    expect(component.find(Menu).at(0).props().open).toBe(true);
  });
  it('should close profile menu ', () => {
    ...
    menuComponent.find(MenuItem).first().simulate('click');
    
    //assert the profile menu is closed
    expect(component.find(Menu).at(1).props().open).toBe(false);  
  });
});
